...
import type { BlockType, CtaBehavior, InterfaceBlock, Pattern, Plan, Requirement, RequirementBucket, RequirementStatus, Schema } from '../types/schema';
...
const patternKeywords: Record<Pattern, string[]> = { pricing: ['pricing', 'price', 'plans', 'tiers', 'subscription', 'billing'], dashboard: ['dashboard', 'cockpit', 'analytics', 'metrics', 'kpi', 'revenue'], settings: ['settings', 'preferences', 'toggles', 'security'], checkout: ['checkout', 'cart', 'payment', 'purchase'], chat: ['chat', 'messages', 'inbox', 'support'], calendar: ['calendar', 'booking', 'schedule', 'slots'], visualization: ['globe','map','marker','location','base','country','city','spinning','rotating','orbit','3d','geographic','latitude','longitude','visualization','diagram','timeline'], custom: [] };
...
function detectPattern(prompt: string): DetectedPattern { const n = prompt.toLowerCase(); if (n.includes('custom')) return { pattern: 'custom', scores: { pricing: 0, dashboard: 0, settings: 0, checkout: 0, chat: 0, calendar: 0, visualization: 0, custom: 10 }, reasons: [{ start: n.indexOf('custom'), end: n.indexOf('custom') + 6, text: 'custom', reason: 'keyword:custom', source: 'pattern_detector' }] };  const scores: Record<Pattern, number> = { pricing: 0, dashboard: 0, settings: 0, checkout: 0, chat: 0, calendar: 0, visualization: 0, custom: 0 }; const reasons: Span[] = []; (Object.keys(patternKeywords) as Pattern[]).forEach((p) => { patternKeywords[p].forEach((kw) => { const match = new RegExp(`\\b${escapeRegex(kw)}\\b`, 'g').exec(n); if (match) { const idx = match.index; scores[p] += kw.length > 6 ? 2 : 1; reasons.push({ start: idx, end: idx + kw.length, text: kw, reason: `keyword:${p}`, source: 'pattern_detector' }); } }); }); const winner = (Object.entries(scores).sort((a, b) => b[1] - a[1])[0]?.[0] as Pattern) || 'custom'; return { pattern: scores[winner] > 0 ? winner : 'custom', scores, reasons }; }
...
export function buildSchema(prompt: string): Schema {
  ...
  const normalized = normalizePrompt(optimization.optimized);
