export type Pattern = 'pricing' | 'dashboard' | 'settings' | 'checkout' | 'chat' | 'calendar' | 'custom' | 'visualization' | 'kanban' | 'onboarding' | 'editor' | 'terminal' | 'social' | 'orchestrator' | 'health' | 'admin';
...
export type BlockType = 'hero' | 'cardGrid' | 'pricingCards' | 'comparisonMatrix' | 'billingToggle' | 'enterpriseContact' | 'metricGrid' | 'activityFeed' | 'settingsControls' | 'chatThread' | 'calendarSlots' | 'checkoutSummary' | 'onboardingSteps' | 'proofStrip' | 'ctaBand' | 'customRequirementGrid' | 'globeVisualization' | 'mapVisualization' | 'geoMarkerLayer' | 'markerLegend' | 'visualControls' | 'animationControls' | 'datasetNotice' | 'dataCoverageBadge' | 'kanbanBoard' | 'codeEditor' | 'codePreview' | 'onboardingProgress' | 'tradingControls' | 'orderBook' | 'candlestickChart' | 'tickerFeed' | 'controlsInteractions' | 'dataVisualization' | 'timelineControls' | 'socialFeed' | 'platformMetrics' | 'engagementChart' | 'dmInbox' | 'agentPipeline' | 'agentLogFeed' | 'tokenCostGrid' | 'promptEditor' | 'healthMetrics' | 'workoutCalendar' | 'healthTrend' | 'goalTracker' | 'tenantList' | 'featureFlagPanel' | 'billingOverview' | 'auditLog' | 'audioVisualizer3D' | 'frequencyControls' | 'threatRadar' | 'section' | 'card' | 'columnLayout' | 'list' | 'table' | 'statGrid' | 'form' | 'toolbar' | 'timeline' | 'detailPanel';
...
export type Schema = {
  ...
  requirementMappingStatus: string;
  contentEntities: string[];
  valuesPricesMetrics: string[];
  controlsInteractions: string[];
  ...
  directives: string[];
  custom: { header: string[]; body: string[]; controls: string[]; ctas: string[] };
  interactive: ControlModel;
  visualization?: VisualizationMeta;
  dynamicAST?: ComponentAST;
  motionConfig?: { preset: string; staggerMs: number; springBounciness: number };
  generationMeta?: {
    ...
    intentGraph?: { primaryIntent: string; nodeCount: number; secondaryIntents?: string[] };
    _pendingDesignPatch?: { token: string; value: string | number };
  };
};
