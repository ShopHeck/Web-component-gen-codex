...
export default function App() {
  ...
  }} /><section className="workspace"><aside className="rail"><Section title="Prompt" n="01" /><p>Templates + draft prompt are local. Generated prompt is frozen until you click Generate.</p><textarea value={draftPrompt} onChange={(e) => setDraftPrompt(e.target.value)} /><p>Draft prompt is editable. Generate to lock it into the current generated prompt.</p><div className="chips"><Chip label="Pattern" value={draftSchema.pattern} /><Chip label="Strategy" value={draftSchema.strategy} /></div><Section title="Templates" n="02" /><p>Choose a template, then click Use + Generate to refresh the working schema.</p><div className="templates">{templates.map((t) => <button key={t[0]} className={active === t[0] ? 'active' : ''} onClick={() => {
    setActive(t[0]);
