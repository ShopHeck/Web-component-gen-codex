...
export default function App() {
  ...
  }} />featured</label></div>}{selected?.type === 'block' && <div className="templates"><input value={schema.blocks[Number(selected.blockId)]?.title ?? ''} onChange={(e) => setWorkingSchema(updateBlock(schema, selected.blockId, { title: e.target.value }))} /><button onClick={() => setWorkingSchema(toggleBlockVisibility(schema, selected.blockId))}>Toggle visibility</button><button onClick={() => setWorkingSchema(moveBlock(schema, Number(selected.blockId), Math.max(0, Number(selected.blockId) - 1)))}>Move up</button></div>}{selected?.type === 'item' && <input value={schema.blocks[Number(selected.blockId)]?.items?.[Number(selected.itemId)] ?? ''} onChange={(e) => setWorkingSchema(updateBlockItem(schema, selected.blockId, selected.itemId, e.target.value))} />} {selected?.type === 'requirement' && <input value={schema.requirements[Number(selected.requirementId)]?.label ?? ''} onChange={(e) => setWorkingSchema(updateRequirement(schema, selected.requirementId, { label: e.target.value }))} />}<div className="templates"><input value={design.text} onChange={(e) => setDesign(updateDesignToken(design, 'text', e.target.value))} /><input value={design.highlight} onChange={(e) => setDesign(updateDesignToken(design, 'highlight', e.target.value))} /></div><Section title="Export" n="05" /><p>Export package is local-first and deterministic. No backend or remote API calls.</p><button onClick={copy}><Copy size={16} />{copied ? 'Copied' : 'Copy export package'}</button><button onClick={onDownloadZip}><Download size={16} />{zipStarted ? 'ZIP download started' : 'Download ZIP package'}</button></aside></section></main>;
}
